...
// ==========================================
const StudentTabs = () => {
  // 🔥 STATE to hold our badge counts
  const [badges, setBadges] = useState({ homework: 0, updates: 0 });

  // 🔥 LISTENER to update badges whenever Dashboard fetches new data
  useEffect(() => {
    const listener = DeviceEventEmitter.addListener('updateStudentBadges', (counts) => {
      setBadges(counts);
    });
  ...
  return (
    ...
    >
      ...
      {/* 🔥 ADDED BADGES & LISTENERS TO CLEAR ON VIEW */}
      <Tab.Screen 
        name="Homework" 
        component={HomeworkListScreen} 
        listeners={{
          focus: () => setBadges(prev => ({ ...prev, homework: 0 }))
        }}
        options={{
          tabBarBadge: badges.homework > 0 ? badges.homework : undefined,
          tabBarBadgeStyle: styles.badgeStyle
        }}
      />
      <Tab.Screen 
        name="Updates" 
        component={StudentUpdatesScreen} 
        listeners={{
          focus: () => setBadges(prev => ({ ...prev, updates: 0 }))
        }}
        options={{
          tabBarBadge: badges.updates > 0 ? badges.updates : undefined,
          tabBarBadgeStyle: styles.badgeStyle
        }}
      />
...
const styles = {
  ...
  // 🔥 ADDED BADGE STYLING (Matches WhatsApp)
  badgeStyle: {
    ...
    minWidth: 18, 
    height: 18, 
    lineHeight: 17, // Centers the text vertically
    alignSelf: 'center'
  }
};
